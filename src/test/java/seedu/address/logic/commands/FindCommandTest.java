...
import static seedu.address.testutil.TypicalPersons.BELT2;
import static seedu.address.testutil.TypicalPersons.BELT3;
...
public class FindCommandTest {
    ...
    public void execute_zeroKeywords_noPersonFound() {
        String expectedMessage = String.format(MESSAGE_APPARELS_LISTED_OVERVIEW, 0);
        NameContainsKeywordsPredicate predicate = preparePredicate(" ");
        FindCommand command = new FindCommand(predicate);
        expectedModel.updateFilteredApparelList(predicate);
        assertCommandSuccess(command, model, commandHistory, expectedMessage, expectedModel);
        assertEquals(Collections.emptyList(), model.getFilteredApparelList());
    }
