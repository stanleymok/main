...
public class SelectCommandTest {
    ...
    public void execute_validIndexUnfilteredList_success() {
        Index lastPersonIndex = Index.fromOneBased(model.getFilteredApparelList().size());

        assertExecutionSuccess(INDEX_FIRST_APPAREL);
        assertExecutionSuccess(INDEX_THIRD_APPAREL);
        assertExecutionSuccess(lastPersonIndex);
    }

    @Test
    public void execute_invalidIndexUnfilteredList_failure() {
        Index outOfBoundsIndex = Index.fromOneBased(model.getFilteredApparelList().size() + 1);

        assertExecutionFailure(outOfBoundsIndex, Messages.MESSAGE_INVALID_APPAREL_DISPLAYED_INDEX);
    }
    ...
    @Test
    public void equals() {
        SelectCommand selectFirstCommand = new SelectCommand(INDEX_FIRST_APPAREL);
        SelectCommand selectSecondCommand = new SelectCommand(INDEX_SECOND_APPAREL);

        // same object -> returns true
        assertTrue(selectFirstCommand.equals(selectFirstCommand));

        // same values -> returns true
        SelectCommand selectFirstCommandCopy = new SelectCommand(INDEX_FIRST_APPAREL);
     ...
     * and checks that the model's selected apparel is set to the apparel at {@code index} in the filtered apparel list.
     */
    private void assertExecutionSuccess(Index index) {
        SelectCommand selectCommand = new SelectCommand(index);
        String expectedMessage = String.format(SelectCommand.MESSAGE_SELECT_PERSON_SUCCESS, index.getOneBased());
        expectedModel.setSelectedPerson(model.getFilteredApparelList().get(index.getZeroBased()));
