...
. Modify link:{repoURL}/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `data/fashionmatch.json` so that the application will load the sample data when you launch it.)
...
* wishes for an app to store their wardrobe and recommends clothes
...
(For all use cases below, the *System* is the `Fashion Match Application` and the *Actor* is the `user`, unless specified otherwise)
...
[none]
* 1a. Entry with same apparel name already exists in Application.
+
[none]
